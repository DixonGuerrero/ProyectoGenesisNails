...
class Controller
{
    ...
    public function cargarImagen($nombre, $carpeta, $nombreFoto)
    {
        ...
        if (
            ...
            try {
                ...
                error_log('Controller::cargarImagen -> Error al cargar la imagen: ' . $e->getMessage());
    ...
    public function eliminarImagen($nombre, $carpeta)
    {
        error_log('Controller::eliminarImagen -> Eliminando imagen: ' . $nombre . ' de la carpeta: ' . $carpeta);

        $directorioImagenes = $_SERVER['DOCUMENT_ROOT'] . '/Proyectos/ProyectoGenesisNails2/assets/images/' . $carpeta . '/';

        error_log('Controller::eliminarImagen -> Directorio: ' . $directorioImagenes);
        try {
            if (file_exists($directorioImagenes . $nombre)) :
                chmod($directorioImagenes . $nombre, 0777);

                return unlink($directorioImagenes . $nombre);

            endif;

            return false;
        } catch (Exception $e) {
            error_log('Controller::eliminarImagen -> Error al eliminar la imagen: ' . $e->getMessage());
